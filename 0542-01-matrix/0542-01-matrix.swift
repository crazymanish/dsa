    ...
    func updateMatrix(_ mat: [[Int]]) -> [[Int]] {
        ...
        for row in 0..<rowsCount {
            for column in 0..<columnsCount {
                ...
                // Top + left
                if row > 0 { 
                    outputMatrix[row][column] = outputMatrix[row-1][column] + 1 
                }
                
                if column > 0 { 
                    outputMatrix[row][column] = min(outputMatrix[row][column], outputMatrix[row][column-1] + 1) 
                } 
            }
        }
        ...
        for row in (0..<rowsCount).reversed() {
            for column in (0..<columnsCount).reversed() {
                ...
                // Down + Right
                if row < rowsCount - 1 { 
                    outputMatrix[row][column] = min(outputMatrix[row][column], outputMatrix[row+1][column] + 1) 
                }
