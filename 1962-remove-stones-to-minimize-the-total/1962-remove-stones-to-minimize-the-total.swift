class Solution {
    func minStoneSum(_ piles: [Int], _ k: Int) -> Int {
        ...
        for pile in piles {
            priorityQueue.enqueue(pile)
            totalStones += pile
        }
        
        var steps = k
        while steps > 0 {
            let pile = priorityQueue.dequeue()!
            let halfPile = pile / 2 // we need to make it half in every step
            let remainingPile = pile - halfPile
            priorityQueue.enqueue(remainingPile)
            
            totalStones -= halfPile
