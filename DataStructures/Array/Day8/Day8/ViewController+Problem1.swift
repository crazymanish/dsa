...
extension ViewController {
    func rearrange(inputArray: [Int], size: Int) {
        print("Input: \(inputArray.joined)")
        print("Performing rearrange such that arr[i] = i")

        var outputArray = inputArray
        var index = 0

        while index < size {
            let indexValue = outputArray[index]

            if indexValue > 0 && indexValue != index {
                let element = outputArray[indexValue]
                outputArray[indexValue] = indexValue
