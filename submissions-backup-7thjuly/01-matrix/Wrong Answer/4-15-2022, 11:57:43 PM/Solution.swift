    ...
    func updateMatrix(_ mat: [[Int]]) -> [[Int]] {
        ...
        for row in (0...lastRow) {
            for column in (0...lastColumn) {
                if mutableMatrix[row][column] == 1 {
                    ...
                    let leftValue = column == 0 ? maxValue : mutableMatrix[row][column-1]
                    let upValue = row == 0 ? maxValue : mutableMatrix[row-1][column]
        ...
        for row in (0...lastRow).reversed() {
            for column in (0...lastColumn).reversed() {
                if mutableMatrix[row][column] == 1 {
                    let rightValue = column == lastColumn ? maxValue : mutableMatrix[row][column+1]
                    let downValue = row == lastRow ? maxValue : mutableMatrix[row+1][column]
