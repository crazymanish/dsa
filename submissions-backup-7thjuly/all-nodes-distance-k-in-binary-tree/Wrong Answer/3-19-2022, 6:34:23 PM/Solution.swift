...
class Solution {
    func distanceK(_ root: TreeNode?, _ target: TreeNode?, _ k: Int) -> [Int] {
        ...
        let leftDepth = k-targetNodeDepth
        let rightDepth = k+targetNodeDepth
        
        if leftDepth == rightDepth {
            return depthWithValuesMap[leftDepth] ?? []
        } else {
            return (depthWithValuesMap[leftDepth] ?? []) + (depthWithValuesMap[rightDepth] ?? [])  
        }
    }
}
